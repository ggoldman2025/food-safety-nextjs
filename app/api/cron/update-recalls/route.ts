    ...
    try {
      ...
      for (const recall of fdaRecalls) {
        const recallData = {
          ...
          source: 'FDA' as const,
          title: recall.productDescription?.substring(0, 255) || 'Unknown Product',
          productDescription: recall.productDescription,
          reasonForRecall: recall.reasonForRecall,
          companyName: recall.recallingFirm || recall.distributionPattern || 'Unknown Company',
          recallInitiationDate: new Date(recall.recallInitiationDate),
          reportDate: recall.reportDate ? new Date(recall.reportDate) : null,
          classification: recall.classification,
          distributionPattern: recall.distributionPattern,
          state: recall.state || null,
          productType: recall.productType || 'Food',
